...
export default function RoomTest({ initialRoom, roomId }) {
  ...
  const fetchRoomUsers = async () => {
    try {
      ...
      // 유저가 방에 남아있는지 확인
      const userStillInRoom = updatedRoom.users.some(
      ...
      // ready / pickup 체크
      const everyoneReady = updatedRoom.users.every(
        (user) => user.ready === true
      );
      const everyonePickedUp = updatedRoom.users.every(
  ...
  const kickButton = async (targetUserId) => {
    ...
    try {
      ...
      const updatedUsers = room.users.filter(
        (u) => Number(u.userId) !== Number(targetUserId)
      );
      ...
      <div className={styles.roomContentWrapper}>
        ...
        <div className={styles.leftColumn}>
          <div className={styles.titleWrapper}>
            ...
            <button
              ...
              onClick={() =>
                handleLeaveRoom({ room, user, roomId, status, navigate })
              }
            >
              <img
              ...
              <ul className={styles.memberList}>
                {room?.users?.length > 0 ? (
                  room.users.map((member, idx) => {
                    const isLeader =
                      room?.leaderId &&
                      member.userId.toString() === room.leaderId.toString();
        ...
        <div className={styles.menuWrapper}>
          ...
          <div className={styles.cartWrapper}>
            <div className={styles.cartTitle}>담은메뉴</div>
            {cart.length > 0 ? (
              cart.map((item) => (
          ...
          <div className={styles.fixedButtonWrapper}>
            {status === "배달중" ? (
              <button className={styles.readyButton} onClick={pickedUp}>
                픽업완료
              </button>
            ) : room?.users?.find((u) => Number(u.userId) === Number(user.id))
                ?.ready ? (
              <button className={styles.readyButton} onClick={cancelOrder}>
                준비취소
              </button>
            ) : (
              <button
                className={styles.readyButton}
                onClick={() => handleReadyToggle(user.id)}
              >
                준비완료
              </button>
            )}

  ...
  useEffect(() => {
    ...
    const url =
      selectedCategoryId === 1
        ? `/api/store/all`
        : `/api/store/categoryId/${selectedCategoryId}`;
      ...
      <div className={styles["circle_category_outer"]}>
        ...
        <div className={styles["circle_category_wrap"]} ref={categoryRef}>
          {category.map((item) => (
            <div
              key={item.id}
              onClick={() => navigate(`/storelist/${item.id}`)}
              className={
        ...
        <button className={styles["scroll_button"]} onClick={scrollRight}>
          <img
            src="https://s3.us-east-1.amazonaws.com/delivery-bucket2025.08/imgfile/main_img/backbtn2.png"
            alt="오른쪽"
          />
        </button>
      </div>
                ...
                <div className={styles["img_explain_wrap"]}>
                  ...
                  <div className={styles["storeexplain"]}>
                    ...
                    <div className={styles["score"]}>
                      가게 위치: {item.storeAddress}
                    </div>
                    <div className={styles["score"]}>
                      배달비:{" "}
                      <span className={styles["deliveryfree"]}>무료배달</span>
                    </div>
                    <div className={styles["score"]}>
                      최소 주문: {item.minPrice}원
                    </div>
                  </div>
                </div>
              </Link>
